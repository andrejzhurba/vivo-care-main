...
function UnderpadsPage() {
  ...
  const productSchema = {
    ...
    description:
      "Гігієнічні поглинаючі пелюшки Vivo Care з 5 шарами захисту для догляду за хворими та дітьми.",
    brand: {
      "@type": "Brand",
      name: "Vivo Care",
    },
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 justify-items-center">
              {underpadSizes.map((item) => {
                ...
                return (
                  <article
                    key={item.id}
                    className="w-full max-w-sm bg-white border border-slate-100 rounded-[2.5rem] p-8 shadow-sm transition-all hover:shadow-xl hover:-translate-y-2 hover:border-blue-100 group flex flex-col"
                  >
            ...
            <div className="max-w-4xl mx-auto">
              ...
              <div className="grid md:grid-cols-2 gap-x-12 gap-y-4 bg-slate-50 p-10 rounded-[3rem] border border-slate-100 shadow-inner">
                {commonSpecs.map((s, i) => (
                  <div
                    key={i}
                    className="flex justify-between items-center py-3 border-b border-slate-200/50"
                  >
